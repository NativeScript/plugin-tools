...
module.exports = (env) => {
  ...
  const tsxRule = baseConfig.module.rules.find(
    (rule) => rule.use && rule.use.loader === 'ts-loader'
  );
  tsxRule.test = /\.(ts|tsx)$/;
  tsxRule.use = [
    /**
   ...
   */
  const nativeScriptDevWebpackHotLoader = baseConfig.module.rules.find(
    (rule) => rule.use === '@nativescript/webpack/hmr/hot-loader'
  );
  ...
  baseConfig.resolve.alias['@demo/shared'] = resolve(
    __dirname,
    '..',
    '..',
    'tools',
  ...
  const existingDefinePlugin = baseConfig.plugins.find(
    (plugin) =>
