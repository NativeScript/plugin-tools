...
export default function (schema: Schema): Rule {
  ...
  return chain([
    ...
    (tree: Tree, context: SchematicContext) => {
      ...
      const focusTargets = (focusPackages && focusPackages.length
        ? focusPackages
        : allPackages
      )
        .map((n) => `\n${getNpmScope()}/${n}`)
      ...
      context.logger.info(
        `${
          isFocusing ? 'Focusing workspace on:' : 'Resetting workspace for:'
        }\n${focusTargets}\n\n`
      );
      if (!schema.ignoreDemos) {
        context.logger.info(
          ` > NOTE: Clean the demo app you plan to test with before running now that the demo code has been updated.\n`
        );
      }
    },
  ]);
