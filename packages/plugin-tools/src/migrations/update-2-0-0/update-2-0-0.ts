...
function updatePackages(tree: Tree) {
  ...
  "compilerOptions": {
    ...
    "rootDir": "."
      ...
      tree.write(`${packageDir}/angular/tsconfig.json`, tsconfgNg);
    }
  }
}
  ...
  updateJson(tree, 'workspace.json', (json) => {
    ...
    for (const project in json.projects) {
      if (json.projects[project].projectType === 'application') {
        ...
        if (json.projects[project].architect && json.projects[project].architect.build && json.projects[project].architect.build.options && json.projects[project].architect.build.options.main) {
          const mainIndex = json.projects[project].architect.build.options.main;
          if (mainIndex.indexOf('index.d.ts') === -1) {
            ...
            json.projects[project].architect.build.options.main = mainIndex.replace('index.ts', 'index.d.ts');
          }
        }
      }
    }
