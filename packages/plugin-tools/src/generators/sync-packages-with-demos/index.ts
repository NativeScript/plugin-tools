...
export default function (
  ...
  if (schema) {
    if (schema.types) {
      ...
      const demoTypes = <Array<SupportedDemoType>>(
        sanitizeCollectionArgs(schema.types)
      );
      for (const t of demoTypes) {
        if (!SupportedDemoTypes.includes(t)) {
          throw new SchematicsException(
            `Can only update supported demo types: ${SupportedDemoTypes.join()}`
          );
        }
      }
      ...
      switch (type) {
        case 'xml':
          ...
          if (indexViewContent.indexOf(`Button text="${name}"`) === -1) {
            ...
            const lastBtnLocatorIndex = indexViewContent.lastIndexOf(
              'view-demo'
            );
            // get final content after that last button
            const remainingContent = indexViewContent.substr(
              lastBtnLocatorIndex,
              indexViewContent.length
            );
            // get first line break to determine position of where to insert next button
            const firstLB = remainingContent.indexOf('\n');
            const endingContent = indexViewContent.substring(
              lastBtnLocatorIndex + firstLB,
              indexViewContent.length
            );
            // context.logger.info('buttonMarkup: ' + buttonMarkup);
            indexViewContent =
              indexViewContent.substring(0, lastBtnLocatorIndex + firstLB) +
              `\n${buttonMarkup}` +
              endingContent;
          }
    ...
    for (const name of getPackageNamesToUpdate()) {
      ...
      if (!tree.exists(demoSharedIndex)) {
        ...
        const templateSource = apply(
          url(`${relativePrefix}files_demo_shared`),
          [
            template({
              name,
              npmScope: getNpmScope(),
